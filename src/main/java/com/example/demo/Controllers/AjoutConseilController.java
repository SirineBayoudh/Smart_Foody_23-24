...
public class AjoutConseilController implements Initializable {
    ...
    void ajoutConseil(ActionEvent event) {
        try {
            // Check if the client has reached the maximum limit of adds per day
            if (hasReachedMaxAddsPerDay()) {
                showAlert(Alert.AlertType.ERROR, "Erreur", "Vous avez atteint le maximum de 3 demandes par jour.");
                return; // Stop execution
            }
            if (demandeLabel.getText()==null || demandeLabel.getText().isEmpty()) {
                showAlert(Alert.AlertType.ERROR, "Erreur", "Veuillez remplir tous les champs.");
                return; // Stop execution
            }
            String insert = "INSERT INTO conseil(id_client, statut, demande, reponse, date_conseil) VALUES (?, ?, ?, ?, ?)";
            con = MyConnection.instance.getCnx();
            st = con.prepareStatement(insert);
            st.setString(1, String.valueOf(id));
            st.setString(2, "en attente");
            st.setString(3, demandeLabel.getText());
            st.setString(4, "");
    ...
    private boolean hasReachedMaxAddsPerDay() throws SQLException {
        LocalDate currentDate = LocalDate.now();
        Date date = Date.valueOf(currentDate);

        String query = "SELECT COUNT(*) FROM conseil WHERE id_client = ? AND date_conseil = ?";
        con = MyConnection.instance.getCnx();
    ...
    Utilisateur user1 = new Utilisateur(1,"Trabelsi","Yassine","Conseiller");
    Utilisateur user2 = new Utilisateur(2,"Miled","Youssef","Conseiller");
    ...
    void getData(MouseEvent event) {
        if (event.getClickCount() == 1) {
            ...
            if (selectedConseil != null) {
                ...
                spinnerNote.getValueFactory().setValue(selectedConseil.getNote());
            }
        }
    ...
    void updateConseil(ActionEvent event) {
        try {
            int noteValue = spinnerNote.getValue();
            ...
            alert.showAndWait().ifPresent(response -> {
                if (response == ButtonType.OK) {
                    ...
                    try {
                        ...
                        st.setString(1, String.valueOf(noteValue));
